...
async def new_user(message: types.Message):
    if str(message.from_user.id) not in await get_all_id():
        ...
        conn.commit()
        await log_add(message.from_user.id, 'Новый пользователь', datetime.datetime.now(), f"user_id = {message.from_user.id}")
...
""", reply_markup=keyboard.as_markup())
    await log_add(message.from_user.id, 'Профиль', datetime.datetime.now(), f"user_id = {message.from_user.id}")
...
async def edit_info(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await log_add(message.from_user.id, 'Добавить инфу', datetime.datetime.now(), f"user_id = {message.from_user.id}; info = {info[0]}{info[1]}{info[2]}")

async def help(message: types.Message):
    await message.answer('С вопросами и предложениями обращайтесь к @Albertt1001')
    await log_add(message.from_user.id, 'Помощь', datetime.datetime.now(), f"user_id = {message.from_user.id}")
...
async def my_books(message: types.Message):
    ...
    await log_add(message.from_user.id, 'Мои книги', datetime.datetime.now(), f"user_id = {message.from_user.id}; books = {', '.join(str_books)}")